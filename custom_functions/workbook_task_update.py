def workbook_task_update(task_name=None, note_title=None, note_content=None, status=None, owner=None, container_id=None, **kwargs):
    """
    ...
    if task_name:
        ...
        if task_count > 1:
            ...
            if task_is_note_required and not note_content and status == 'complete' and len(task_notes) == 0:
                ...
                if note_content:
                    if note_title:
                        success, message, note_id = phantom.add_note(container=container_id, note_type='task', task_id=task_id, title=note_title, content=note_content, note_format='markdown')
                        outputs['note_id'] = str(note_id)
                    else:
                        success, message, note_id = phantom.add_note(container=container_id, note_type='task', task_id=task_id, content=note_content, note_format='markdown')[1] 
                        outputs['note_id'] = str(note_id)
                elif note_content:
