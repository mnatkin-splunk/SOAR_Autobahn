...
def merge_hostnames(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    filtered_artifacts_data_0 = phantom.collect2(container=container, datapath=['filtered-data:host_filter:condition_1:artifact:*.cef.destinationAddress', 'filtered-data:host_filter:condition_1:artifact:*.cef.destinationHostName', 'filtered-data:host_filter:condition_1:artifact:*.cef.dest', 'filtered-data:host_filter:condition_1:artifact:*.cef.dest_ip', 'filtered-data:host_filter:condition_1:artifact:*.cef.dest_nt_host', 'filtered-data:host_filter:condition_1:artifact:*.cef.sourceAddress', 'filtered-data:host_filter:condition_1:artifact:*.cef.sourceHostName', 'filtered-data:host_filter:condition_1:artifact:*.cef.src', 'filtered-data:host_filter:condition_1:artifact:*.cef.src_ip', 'filtered-data:host_filter:condition_1:artifact:*.cef.src_nt_host'])
    ...
    phantom.custom_function(custom_function='enrichment/list_merge_dedup', parameters=parameters, name='merge_hostnames', callback=get_process_info_spl)
